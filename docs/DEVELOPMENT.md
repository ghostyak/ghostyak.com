...
320px·768px·데스크톱에서 가로 넘침, 제목·버튼 줄바꿈, FAQ 키보드 조작과 다운로드 앵커를 확인한다. 링크 복사는 성공 메시지와 실패 시 직접 복사할 주소 입력란을 제공한다. 2026-09-07 승인된 원문은 공개 사전의 `landing` 키에 연결되어 있다. 수정 시 `src/i18n/landing/ko.ts`를 먼저 확정한 뒤 나머지 8개 언어를 맞춘다. 공개 `/`, `/product/boxes`는 같은 컴포넌트를 사용한다. 기존 다운로드 경로는 같은 언어의 `/#download`로 이동한다.

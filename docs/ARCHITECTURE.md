...
현재 홈페이지는 루트 레이아웃과 페이지, 헤더·푸터·기능 카드·에디션 카드, 제품 데이터 모듈로 구성한다. 제품 데이터는 Free와 Pro 평가판의 설명 및 다운로드 URL을 함께 관리한다. `src/app/sitemap.ts`와 `src/app/robots.ts`는 검색엔진용 `/sitemap.xml`과 `/robots.txt`를 생성한다. API와 데이터베이스 계층은 결제 및 라이선스 작업을 시작할 때 추가한다.

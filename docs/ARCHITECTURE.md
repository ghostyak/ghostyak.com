...
- `/`: 세계시계 중심 고정 히어로, 무료 다운로드·제품 상세 링크, 활용 소개, Boxes 제품 카드와 블로그 진입점
...
`WorldClockPreview`는 홈과 Boxes 상세에서 재사용하는 Server Component다. 고정된 예시 시점을 도시별 IANA 시간대로 변환하고 `Intl.DateTimeFormat`으로 날짜와 시간을 현지화한다. 실시간 갱신이나 조작 기능은 없으며 미리보기임을 명시한다. 홈과 제품 메타데이터에도 세계시계 소개를 반영하고 제품 구조화 데이터의 기능 목록에 세계시계를 포함한다.

...
export function ProductLanding({ id, name, badges, copy, screenshots, featureIcons, highlightIcon: HighlightIcon, productIcon: ProductIcon, downloadUrl, repository, viewScreenshot }: {
  id: string;
  name: string;
  badges: ReactNode;
  ...
  return <main id="main-content" className="[overflow-wrap:anywhere]">
    ...
    <section id="download" className="surface-ink scroll-mt-24 px-4 py-16 sm:px-8 sm:py-24" aria-labelledby={`${id}-download`}>
      <div className="mx-auto flex max-w-7xl flex-col items-start gap-8 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-col items-start gap-2 md:items-end">{downloadLink}<a href={repository.url} className={textLink} target="_blank" rel="noreferrer">{repository.label}<ArrowUpRight className="size-3.5" aria-hidden="true" /></a></div>
      </div>
    </section>
  </main>;
}

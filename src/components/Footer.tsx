  ...
  if (icon === "instagram") {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <circle cx="17.4" cy="6.7" r=".8" className="social-icon-fill" />
      </svg>
    );
  }
  ...
  return (
    <footer className="site-footer">
      <div className="container footer-inner">
        ...
        <nav className="social-links" aria-label={labels.socials}>
          {socialLinks.map(({ href, icon, label }) => (
            <a key={icon} href={href} target="_blank" rel="noreferrer" aria-label={label} title={label}>
